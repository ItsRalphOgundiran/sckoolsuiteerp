...
export function buildSchoolRoleModel(roleScope: Exclude<RoleScope, "superadmin">, core: CoreData): RoleDashboardModel {
  const totalInvoiceAmount = core.invoices.reduce((sum, invoice) => sum + invoice.totalAmount, 0);
  const totalPaidAmount = core.payments.reduce((sum, payment) => sum + payment.amount, 0);
  ...
  const models: Record<Exclude<RoleScope, "superadmin">, RoleDashboardModel> = {
    admin: {
      ...
      stats: [
        { label: "Active Session", value: core.sessions.find((item) => item.isCurrent)?.name ?? "N/A", hint: "Academic session" },
        { label: "Active Term", value: core.terms.find((item) => item.isCurrent)?.name ?? "N/A", hint: "Academic term" },
        { label: "Total Students", value: String(core.students.length), hint: "Enrolled learners" },
        { label: "Total Teachers", value: String(core.teachers.length), hint: "Instruction workforce" },
        { label: "Total Parents", value: String(core.parents.length), hint: "Linked guardians" },
        { label: "Total Classes", value: String(core.classes.length), hint: "Active class groups" },
        { label: "Fee Collection", value: `${collectionRate.toFixed(1)}%`, hint: "Invoice recovery" },
        { label: "Unpaid Invoices", value: String(core.invoices.filter((item) => item.balance > 0).length), hint: "Outstanding balances" },
      ],
      quickActions: [
      ...
      activities: core.students.slice(0, 6).map((student) => ({ id: student.id, title: student.user.name, detail: "New registration", time: formatDate(student.createdAt) })),
      tasks: [
        { id: "res", title: "Pending results", detail: `${Math.max(0, core.students.length - core.scores.length)} students pending score rows`, time: "Today" },
        { id: "fees", title: "Follow up unpaid invoices", detail: `${core.invoices.filter((item) => item.balance > 0).length} invoices open`, time: "This week" },
      ],
    ...
    teacher: {
      ...
      stats: [
        { label: "Assigned Classes", value: String(core.classes.length), hint: "Class responsibilities" },
        { label: "Assigned Subjects", value: String(core.subjects.length), hint: "Teaching load" },
        { label: "Pending Scores", value: String(Math.max(0, core.students.length - core.scores.length)), hint: "Entries remaining" },
        { label: "Attendance Logs", value: String(core.attendance.length), hint: "Recorded marks" },
      ],
      quickActions: [
      ...
      series: {
        ...
        data: core.subjects.slice(0, 6).map((subject) => ({
          label: subject.name,
          value: Math.round(core.scores.filter((score) => score.subjectId === subject.id).reduce((sum, score) => sum + score.total, 0) / Math.max(1, core.scores.filter((score) => score.subjectId === subject.id).length)),
        })),
      },
      ...
      tasks: [
        { id: "grade", title: "Grade pending scripts", detail: `${Math.max(0, core.students.length - core.scores.length)} pending score records`, time: "Today" },
    ...
    parent: {
      ...
      stats: [
        { label: "Children", value: String(core.students.length), hint: "Linked to parent profile" },
        { label: "Attendance Logs", value: String(core.attendance.length), hint: "Visible records" },
        { label: "Fee Balance", value: naira(core.invoices.reduce((sum, item) => sum + item.balance, 0)), hint: "Outstanding due" },
        { label: "Latest Results", value: String(core.scores.length), hint: "Published score rows" },
      ],
      quickActions: [
      ...
      series: {
        ...
        data: [
          { label: "Present", value: core.attendance.filter((item) => item.status === "PRESENT").length },
          { label: "Absent", value: core.attendance.filter((item) => item.status === "ABSENT").length },
    ...
    student: {
      ...
      stats: [
        { label: "Current Class", value: core.students[0]?.class?.name ?? "N/A", hint: "Assigned class" },
        { label: "Subjects", value: String(core.subjects.length), hint: "Current term load" },
        { label: "Assignments", value: String(core.assignments.length), hint: "Total assignments" },
        { label: "Attendance", value: String(core.attendance.length), hint: "Attendance records" },
      ],
      quickActions: [
      ...
      activities: core.assignments.slice(0, 6).map((item) => ({ id: item.id, title: item.title, detail: item.subject?.name ?? "Subject", time: formatDate(item.dueDate) })),
      tasks: [
        { id: "submit", title: "Submit pending assignments", detail: `${core.assignments.filter((item) => !item.submittedAt).length} due soon`, time: "Soon" },
        { id: "review", title: "Review latest results", detail: "Check term progress report", time: "Today" },
      ],
      announcements: mapAnnouncements(core.announcements),
      tableTitle: "Assignment Tracker",
      tableRows: core.assignments.slice(0, 20).map((item) => ({ id: item.id, primary: item.title, secondary: item.subject?.name ?? "Subject", status: item.submittedAt ? "SUBMITTED" : "PENDING", date: formatDate(item.dueDate) })),
    },
    accountant: {
      ...
      stats: [
        ...
        { label: "Outstanding", value: naira(core.invoices.reduce((sum, item) => sum + item.balance, 0)), hint: "Unpaid balances" },
        { label: "Overdue Invoices", value: String(core.invoices.filter((item) => item.balance > 0).length), hint: "Debtors list" },
      ],
      quickActions: [
    ...
    registrar: {
      ...
      stats: [
        { label: "New Applications", value: String(core.students.length), hint: "Incoming records" },
        { label: "Pending Approvals", value: String(Math.max(0, core.students.length - core.classes.length)), hint: "Awaiting admission" },
        { label: "Admitted Students", value: String(core.students.length), hint: "Total enrolled" },
        { label: "Incomplete Registrations", value: String(core.students.filter((student) => !student.classId).length), hint: "Need class placement" },
      ],
      quickActions: [
      ...
      series: {
        ...
        data: [
          { label: "Applications", value: core.students.length },
          { label: "Placed", value: core.students.filter((student) => student.classId).length },
          { label: "Incomplete", value: core.students.filter((student) => !student.classId).length },
        ],
      },
      activities: core.students.slice(0, 6).map((student) => ({ id: student.id, title: student.user.name, detail: student.class?.name ?? "Awaiting class", time: formatDate(student.createdAt) })),
      tasks: [
        { id: "place", title: "Complete class placements", detail: `${core.students.filter((student) => !student.classId).length} students pending`, time: "Urgent" },
        { id: "docs", title: "Verify submitted documents", detail: "Review admission uploads", time: "Today" },
      ],
      announcements: mapAnnouncements(core.announcements),
      tableTitle: "Recent Admissions",
      tableRows: core.students.slice(0, 20).map((student) => ({ id: student.id, primary: student.user.name, secondary: student.class?.name ?? "No class yet", status: student.classId ? "ADMITTED" : "PENDING", date: formatDate(student.createdAt) })),
    },
  };

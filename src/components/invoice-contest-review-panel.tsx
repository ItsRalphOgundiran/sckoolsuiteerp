...
type InvoiceContestReviewPanelProps = {
  currentRole?: string;
};
...
export function InvoiceContestReviewPanel({ currentRole }: InvoiceContestReviewPanelProps) {
  const [contests, setContests] = useState<ContestRecord[]>([]);
  const [audits, setAudits] = useState<ContestAudit[]>([]);
  const [loading, setLoading] = useState(false);
  const [toast, setToast] = useState("");
  const [commentByInvoice, setCommentByInvoice] = useState<Record<string, string>>({});
  ...
  async function loadContests() {
    ...
    setToast("");
    try {
      const response = await fetch("/api/admin/invoices/contests", { cache: "no-store" });
  ...
  async function submitReview(invoiceId: string, action: "UNDER_REVIEW" | "APPROVED" | "REJECTED") {
    const contest = contests.find((item) => item.invoiceId === invoiceId);
    if (!contest) return;

    const response = await fetch("/api/admin/invoices/contests/review", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        invoiceId,
        action,
        staffComment: commentByInvoice[invoiceId] ?? "",
        finalAdjustments: [],
      }),
    });
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900/70">
      ...
      <div className="space-y-3">
        {contests.map((contest) => (
          <article key={contest.invoiceId} className="rounded-xl border border-slate-200 p-3 dark:border-slate-700">
            <div className="flex flex-wrap items-start justify-between gap-2">
              <div>
                <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">Bill {contest.invoiceNumber} • {contest.studentName}</p>
            ...
            <div className="mt-2 space-y-1">
              {contest.items.length ? contest.items.map((item) => (
                <div key={item.invoiceItemId} className="grid grid-cols-1 gap-2 rounded-lg bg-slate-50 p-2 text-xs md:grid-cols-[1.4fr_1fr_1fr] dark:bg-slate-800/60">
            ...
            <div className="mt-2 rounded-lg border border-slate-200 bg-slate-50 p-2 text-[11px] dark:border-slate-700 dark:bg-slate-800/40">
              ...
              {audits.filter((row) => row.invoiceId === contest.invoiceId).slice(-5).map((row) => (
              ...
              {!audits.some((row) => row.invoiceId === contest.invoiceId) ? <p className="text-slate-500 dark:text-slate-400">No audit entries yet.</p> : null}
            </div>
            ...
            <textarea
              ...
              value={commentByInvoice[contest.invoiceId] ?? contest.staffComment ?? ""}
              onChange={(e) => setCommentByInvoice((prev) => ({ ...prev, [contest.invoiceId]: e.target.value }))}
            />
            ...
            {(contest.status === "SUBMITTED" || contest.status === "UNDER_REVIEW") ? (
              <div className="mt-2 space-y-2">
                ...
                <button type="button" className="rounded-md border border-slate-300 px-2 py-1 text-xs text-slate-700 hover:bg-slate-50" onClick={() => submitReview(contest.invoiceId, "UNDER_REVIEW")}>Mark Under Review</button>
                <button type="button" className="rounded-md border border-rose-300 bg-rose-50 px-2 py-1 text-xs text-rose-700" onClick={() => submitReview(contest.invoiceId, "REJECTED")}>Reject</button>
                <button
                  type="button"
                  className="rounded-md bg-emerald-600 px-2 py-1 text-xs text-white disabled:cursor-not-allowed disabled:bg-emerald-300"
                  onClick={() => submitReview(contest.invoiceId, "APPROVED")}
                  disabled={!canApprove}
